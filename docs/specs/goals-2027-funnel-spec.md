# 2027 Goals Funnel — spec v0.3 (2026-09-22, Matti's rulings folded in)
...
|---|---|---|---|
| D1 | This funnel vs the Sep 17 offseason spec | **RESOLVED (Matti: "resolve conflict").** The funnel wins the page and the offer: `/goals/` replaces `/offseason/`, and the step-two offer lives in the results screen. The **$150 Offseason Read consult survives** as a separate higher-touch product offered in replies, not on the page. The November broadcast stays **pitch-free** and links to the free questionnaire. That draft is committed here as `offseason-goal-setting-product-spec.md` with a SUPERSEDED header. Earlier wording: this funnel replaces that spec's page and product. The November broadcast stays pitch-free and links to the questionnaire, not an offer. The consult stays a separate, existing product. | Matti asked for a homepage lead magnet with an offer inside the flow; that spec was a draft awaiting his gates. |
| D2 | Price for a whole-season plan | **RULED (Matti, Sep 22): a Season Plan product, not a raised cap.** Proposed: **$449**, the whole year (A/B/C races periodised), **four scheduled rebuilds**, same 7-day refund, email support. Labour budget ~1 h 45 m per athlete per year (≈45 min first build + 4 × 15 min rebuilds) and it only holds if Motoren does the rebuilds. Earlier draft said $499, separate from the race plan (which stays $15/week, 4-week minimum, $249 cap). Up to 52 weeks, every A/B/C race periodised, four scheduled rescales. | Raising the cap breaks the Stripe fixed-price table (17+ weeks is a pre-made $249 price on the shared account) and every "$249" in ~20 files across 3 repos. A separate product avoids both. $499 sits between a $249 race plan and ~$2,600/yr coaching ($199 / 4 weeks). **Money: Matti sets the number before anything ships.** |
...
| D5 | Homepage change | **RULED (Matti, Sep 22): direction D, layout D1 — the poster wall.** Left: three posters at once, the middle one large, two angled behind (rotate the artwork only, never the headline or button, and never on a timer). Right: "It's dreaming season. Most of it stays a dream." + MAKE MINE. The race database moves to a band directly beneath the fold (search, 384/140/15, "Every gravel race, rated →"), not deleted. Matti: people land on race pages, not the homepage — **verified**: Sep 21 sessions 149, top landings `/race/best-gravel-races-colorado/` 10, `/` 8 (~5%); the weekly SEO report's buckets carry race pages and `/guide/`, no homepage row. | Two cautions kept: (1) the homepage still serves direct and AI-referral visitors, so the race band stays; (2) posters shown on the site use real, permissioned athlete goals or are marked SAMPLE — invented athletes would be fabrication. |
| D5b | Where the goal CTA actually earns its keep | **Race pages**, in the Oct–Jan window: a goal strip under the ratings spine ("Planning to do this one in 2027?"). Homepage is the statement; race pages are the traffic. | Same data as D5, plus Fable's point that homepage traffic is the lowest-value slot. |
...
| D17 | Consent line | See "Consent and analytics" below. | Fable. |
| D18 | Pricing lives in one place | **Before the Season Plan**: a single `pricing.json` (products, rates, caps, inclusions) read by the Python generators, the checkout server and a generated JS constant; **Stripe prices built at checkout** instead of the 14 pre-made week-count prices; copy templates take the number from the file; a test that fails on stray money literals; every product registered once with its reconciliation label. | Matti called the current spread a rat's nest: prices live in 5 disconnected places and 41 files. This makes this change, and every later one, a one-line edit. ~1 day. |
| D19 | Voice-recorded walkthrough | After step 2, a `/walkthrough` flow: Matti records audio (optionally screen) while using the page, drops the file in a watched folder, a script transcribes it and turns it into per-section items (friction, copy fixes, bugs, ideas) written to a review file plus papercuts. | Matti's ask. Cheap, and it catches what analytics can't. |
...
4. **Pricing in one place** (D18), then the **Season Plan product** (D2, D15, D16):
   labour budget, price, Stripe product, checkout branch, reconciliation entry, copy.
   The `/walkthrough` feedback loop (D19) lands alongside.
