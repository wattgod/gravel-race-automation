...
(function(){{
  ...
  function setupForms(){{
    var health=document.getElementById('gg-health-form');
    health.addEventListener('submit',function(e){{e.preventDefault();var fd=new FormData(health);submitForm(health,'submit_health_update',{{review_status:fd.get('review_status'),current_information_ack:fd.get('current_information_ack')==='on',emergency_boundary_ack:fd.get('emergency_boundary_ack')==='on'}});}});
