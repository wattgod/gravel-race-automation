...
COUNTRY_CODES = {
    ...
    'Thailand': 'TH', 'Japan': 'JP', 'Rwanda': 'RW',
    'British Columbia': 'CA', 'Ontario': 'CA',
...
(function() {{
  ...
  if (link) {{
    link.addEventListener('click', function() {{
      if (typeof gtag === 'function') {{
        gtag('event', 'goal_hero_click', {{ src: 'race', race_slug: {json.dumps(slug)} }});
      }}
    }});
  }}
}})();
...
def build_email_capture(rd: dict) -> str:
    ...
    <div class="gg-email-capture-inner">
      ...
      <p class="gg-email-capture-text">12-week training timeline, race-day checklists, packing list, and personalized fueling calculator — unlocked instantly.</p>
